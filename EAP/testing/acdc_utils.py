...
def logit_diff_metric(logits, correct_labels, wrong_labels, return_one_element: bool=True) -> torch.Tensor:
    #logit [sample_number, seq_len, dimension]
...
def logit_rank_metric(logits, correct_labels, wrong_labels, return_one_element: bool=True) -> torch.Tensor:
    range = torch.arange(len(logits))

    # Compute ranks for correct logits
    correct_rank = (logits[range, -1, :].argsort(dim=-1, descending=True).argsort(dim=-1) + 1)[range, correct_labels]

    # Compute ranks for incorrect logits
   

    # Note: negative sign so we minimize
    if return_one_element:
            return correct_rank.float().mean()
    else:
            return (correct_rank.float()).view(-1)
        
def logit_top_metric(logits, correct_labels, wrong_labels, return_one_element: bool=True) -> torch.Tensor:
    range = torch.arange(len(logits))

    # Compute ranks for correct logits
    correct_rank = (logits[range, -1, :].argsort(dim=-1, descending=True).argsort(dim=-1) + 1)[range, correct_labels]

    # Compute the number of correct ranks that are equal to 1
    correct_top = (correct_rank == 1).sum()

    # Note: negative sign so we minimize
    if return_one_element:
        return correct_top.float()
    else:
        return correct_rank.float().view(-1)
