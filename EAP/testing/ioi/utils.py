...
from testing.acdc_utils import filter_nodes, get_present_nodes
...
import click
import IPython
from testing.acdc_utils import MatchNLLMetric, frac_correct_metric, logit_diff_metric, kl_divergence, negative_log_probs,logit_rank_metric,logit_top_metric
import torch
from testing.docstring.utils import AllDataThings
from testing.ioi.ioi_dataset import IOIDataset  # NOTE: we now import this LOCALLY so it is deterministic
...
def get_all_ioi_things(num_examples, device, metric_name, patching,kl_return_one_element=True):
    ...
    if patching=="denoising":  
        ...
        ioi_dataset = (
            abc_dataset.gen_flipped_prompts(("IO", "RAND"), seed=1)
            .gen_flipped_prompts(("S", "RAND"), seed=2)
            .gen_flipped_prompts(("S1", "RAND"), seed=3)
        )
    ...
    with torch.no_grad():
        ...
        base_model_logit_corrupted= tl_model(patch_data)[:, -1, :]
        base_model_logprobs_corrupted = F.log_softmax(base_model_logit_corrupted, dim=-1)

    base_validation_logprobs = base_model_logprobs[:num_examples, :]
    base_test_logprobs = base_model_logprobs[num_examples:, :]
    base_validation_logprobs_corrupted =base_model_logprobs_corrupted[:num_examples, :]
    base_test_logprobs_corrupted = base_model_logprobs_corrupted[num_examples:, :]
    ...
    elif metric_name == "logit_top":
        validation_metric = partial(
            logit_top_metric,
            correct_labels=validation_labels,
            wrong_labels=validation_wrong_labels,
        )
    elif metric_name == "logit_rank_corrupted":
        validation_metric = partial(
            logit_rank_metric,
            correct_labels=validation_wrong_labels,
            wrong_labels=validation_labels,
        )
    elif metric_name == "kl_div_corrupted":
        validation_metric = partial(
            kl_divergence,
            base_model_logprobs=base_validation_logprobs_corrupted,
            last_seq_element_only=True,
            base_model_probs_last_seq_element_only=False,
            return_one_element=kl_return_one_element,
        )    
    ...
    elif metric_name == "logit_diff_corrupted":
        validation_metric = partial(
            logit_diff_metric,
            correct_labels=validation_wrong_labels,
            wrong_labels=validation_labels,
    ...
    test_metrics = {
        ...
        "kl_div_corrupted": partial(
            kl_divergence,
            base_model_logprobs=base_test_logprobs_corrupted,
            last_seq_element_only=True,
            base_model_probs_last_seq_element_only=False,
        ),
        "logit_diff": partial(
        ...
        "logit_diff_corrupted": partial(
            logit_diff_metric,
            correct_labels=test_wrong_labels,
            wrong_labels=test_labels,
        ),
        "logit_rank": partial(
            logit_rank_metric,
            correct_labels=test_labels,
