...
try:
    from testing.tracr_task.utils import (
...
from testing.docstring.utils import get_all_docstring_things
from testing.logic_gates.utils import get_all_logic_gate_things
from testing.acdc_utils import (
    make_nd_dict,
    reset_network,
    shuffle_tensor,
    cleanup,
...
from testing.TLACDCCorrespondence import TLACDCCorrespondence
from testing.TLACDCInterpNode import TLACDCInterpNode
from testing.TLACDCExperiment import TLACDCExperiment

from testing.acdc_utils import (
    kl_divergence,
)
from testing.ioi.utils import (
    get_all_ioi_things,
    get_gpt2_small,
)
from testing.induction.utils import (
    get_all_induction_things,
    get_validation_data,
    get_good_induction_candidates,
    get_mask_repeat_candidates,
)
from testing.greaterthan.utils import get_all_greaterthan_things

from testing.acdc_graphics import (
    build_colorscheme,
    show
)
import argparse
...
parser.add_argument('--threshold', type=float, required=False, default="0.0575",help='Value for THRESHOLD')
...
clean_logit= exp.call_metric_with_corr(exp.corr, things.test_metrics["logit_diff"], things.test_data)
print("Clean Logit diff is: ", clean_logit)
corrupted_logit= exp.call_metric_with_corr(exp.corr, things.test_metrics["logit_diff_corrupted"], things.test_patch_data)
print("Corrupted Logit diff is: ", corrupted_logit)

clean_logit_rank= exp.call_metric_with_corr(exp.corr, things.test_metrics["logit_rank"], things.test_data)
print("Clean Logit rank is: ", clean_logit_rank)
corrupted_logit_rank= exp.call_metric_with_corr(exp.corr, things.test_metrics["logit_rank"], things.test_patch_data)
print("Corrupted Logit rank is: ", corrupted_logit_rank)

clean_logit_top= exp.call_metric_with_corr(exp.corr, things.test_metrics["logit_top"], things.test_data)
print("Clean Logit top is: ", clean_logit_top)

clean_kl = exp.call_metric_with_corr(exp.corr, things.test_metrics["kl_div"], things.test_data)
print("Clean KL is: ", clean_kl)

corrupted_kl = exp.call_metric_with_corr(exp.corr, things.test_metrics["kl_div_corrupted"], things.test_patch_data)
print("Corrupted KL is: ", corrupted_kl)

subgraph=torch.load("ioi_bool_new_edges.pth")
exp.remove_subgraph(subgraph)
...
exp.current_node = exp.corr.first_node()
out_top= exp.call_metric_with_corr(exp.corr, things.test_metrics["logit_top"], things.test_data)
print("Top to clean label after removing is: ", out_top)
out_logit= exp.call_metric_with_corr(exp.corr, things.test_metrics["logit_diff"], things.test_data)
print("logit to clean label after removing is: ", out_logit)
out_logit_c= exp.call_metric_with_corr(exp.corr, things.test_metrics["logit_diff_corrupted"], things.test_data)
print("logit to corrupted label after removing is: ", out_logit_c)
out_logit_rank= exp.call_metric_with_corr(exp.corr, things.test_metrics["logit_rank"], things.test_data)
print("rank to clean label after removing is: ", out_logit_rank)
out_logit_rank_c= exp.call_metric_with_corr(exp.corr, things.test_metrics["logit_rank_corrupted"], things.test_data)
print("rank to corrupted label after removing is: ", out_logit_rank_c)
out_kl = exp.call_metric_with_corr(exp.corr, things.test_metrics["kl_div"], things.test_data)
print("KL to clean label after removing is: ", out_kl)
out_kl_c = exp.call_metric_with_corr(exp.corr, things.test_metrics["kl_div_corrupted"], things.test_data)
print("KL  to corrupted label after removing is: ", out_kl_c)
