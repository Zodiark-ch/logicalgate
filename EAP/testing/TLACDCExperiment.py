...
import torch
from testing.acdc_graphics import show
...
from testing.global_cache import GlobalCache
from testing.acdc_graphics import log_metrics_to_wandb
import warnings
import wandb
from testing.acdc_utils import extract_info, shuffle_tensor
from testing.TLACDCEdge import (
    TorchIndex,
    Edge, 
    EdgeType,
...
import time
from testing.acdc_utils import next_key
import json
...
class TLACDCExperiment:
    ...
    def __init__(
        ...
        self.corr = TLACDCCorrespondence.setup_from_model(self.model, use_pos_embed=use_pos_embed)#TLACDCCorrespondence 类用于构建和操作计算图，提供了添加节点和边、移除边、获取所有节点和边等功能。

        if early_exit: 
            return
            
        self.reverse_topologically_sort_corr()#对 TLACDCCorrespondence 对象中的计算图进行拓扑排序，并将排序后的图存储在 self.corr.graph 中
        ...
        self.metric = lambda x: metric(x).item()#这样做的目的是为了在后续代码中使用 self.metric 时，能够直接得到一个标量值，而不是一个张量
    ...
    def save_edges(self, fname):
        ...
        for t, e in self.corr.all_edges().items():
            ...
            edges_list.append((t_serializable, e.effect_size))
    ...
    def step(self, early_stop=False, testing=False):
        ...
        for sender_name in sender_names_list:
            sender_indices_list = list(self.corr.edges[self.current_node.name][self.current_node.index][sender_name])#attention head？
            ...
            for sender_index in sender_indices_list:#每个attention head里的qkv？若是其他则是空
        ...
        self.increment_current_node()#将current_node指向下一个节点
    ...
    def increment_current_node_forregraph(self) -> None:
        while True:
            self.current_node = self.find_next_node()
            #print("We moved to ", self.current_node)
            break
    ...
    def load_subgraph(self, subgraph: Subgraph):
        ...
        for (receiver_name, receiver_index, sender_name, sender_index), is_present in subgraph.items():
            ...
            set_of_edges.add((receiver_name, receiver_torch_index, sender_name, sender_torch_index))
        ...
        while True:
            ...
            for sender_name in sender_names_list:
                ...
                for sender_index in sender_indices_list:#每个attention head里的qkv？若是其他则是空
                    ...
                    cur_parent = self.corr.graph[sender_name][sender_index]
                    # if edge.edge_type == EdgeType.ADDITION:
                    #         added_sender_hook = self.add_sender_hook(cur_parent,)  
                    if (self.current_node.name,self.current_node.index,sender_name,sender_index)not in set_of_edges:
                    #移除当前边并更新度量值
                        edge.present = False
                        self.corr.remove_edge(self.current_node.name, self.current_node.index, sender_name, sender_index) 
                        print('we remove', self.current_node.name, self.current_node.index, sender_name, sender_index)             
                    else:
                        continue
            ...
            if self.current_node is None  or self.current_node.name in ["blocks.0.hook_resid_pre", "hook_pos_embed", "hook_embed"]:
                break        
            self.increment_current_node_forregraph()#将current_node指向下一个节点
        # self.update_cur_metric(recalc_metric=True, recalc_edges=True) 
    def remove_subgraph(self, subgraph: Subgraph):
        ...
        while True:
            ...
            for sender_name in sender_names_list:
                ...
                for sender_index in sender_indices_list:#每个attention head里的qkv？若是其他则是空
                    ...
                    if edge.edge_type == EdgeType.ADDITION:
                            added_sender_hook = self.add_sender_hook(cur_parent,)  
                    if (self.current_node.name,self.current_node.index,sender_name,sender_index) in set_of_edges:
                    #移除当前边并更新度量值
                        edge.present = False
                        self.corr.remove_edge(self.current_node.name, self.current_node.index, sender_name, sender_index)              
                    else:
            ...
            if self.current_node is None  or self.current_node.name in ["blocks.0.hook_resid_pre", "hook_pos_embed", "hook_embed"]:
            #if self.current_node.name not in ["blocks.11.hook_resid_post"]:
                break        
            self.increment_current_node()#将current_node指向下一个节点
        self.update_cur_metric(recalc_metric=True, recalc_edges=True)     
    ...
    def check_bool(self, early_stop=False, testing=False):
        if self.current_node is None:
            return
